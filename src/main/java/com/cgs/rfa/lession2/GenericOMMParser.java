 ...
 * any OMM common that is passed to it using the parse() method.
 *
...
public final class GenericOMMParser {
  ...
  public static FieldDictionary getDictionary(int dictId) {
    ...
    return  DICTIONARIES.get(new Integer(dictId));
  }
  ...
  public static final void parseAnsiPageData(OMMData data, PrintStream ps, int tabLevel) {
    ...
    if (newPage) {
      ps.println(CURRENT_PAGE.toString()); // print the page if it is a refresh common
    } else {
